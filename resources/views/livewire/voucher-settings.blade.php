<div>
    <h1>eVoucher Percentage Settings</h1>
    ...
    <div class="custom-tabs">
        ...
        <li class="nav-item">
            <a class="voucher-nav-link nav-link {{ $activeTab === $network ? 'active' : '' }}" 
               id="{{ strtolower($network) }}-tab"
               wire:click="setActiveTab('{{ $network }}')" 
               role="tab" 
               aria-controls="{{ strtolower($network) }}" 
    ...
    @endforeach
        ...
        <div class="tab-content" id="myTabContent">
            @foreach ($networkNames as $network)
                <div class="voucher-tab-pane tab-pane fade {{ $activeTab === $network ? 'show active' : '' }}" 
                     id="{{ strtolower($network) }}" 
                     role="tabpanel" 
                     aria-labelledby="{{ strtolower($network) }}-tab">
                    <div class="tab-inner-content">
                        ...
                        <form wire:submit.prevent="updatePercentage('{{ $network }}')">
                            <div class="form-group">
                                <label for="{{$network}}-percentage">
                                    Enter {{ $network }} Percentage:
                                </label>
                                <input type="number" class="form-control"
                                       id="{{$network}}-percentage"
