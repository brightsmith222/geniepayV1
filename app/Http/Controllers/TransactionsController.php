...
class TransactionsController extends Controller
{
    ...
    public function vtpassRequeryWebhook(Request $req)
    {
        Log::info('VTpass Callback Payload: ', $req->all());
        ...
        if ($type === 'transaction-update') {
            ...
            $requestCode = $data['code'] ?? null;
            $vtpassTransactionId = $data['content']['transactions']['transactionId'] ?? null;

            $transaction = Transactions::where('transaction_id', $requestId)->first();

            if ($transaction) {

                if ($requestCode === '000') {
                    $transaction->status = 'Successful';
                    $transaction->reference = $vtpassTransactionId;
                    $transaction->save();
                }

                // If the transaction failed and needs to be reversed
                if ($requestCode === '040') {

                    $walletTx = WalletTransactions::where('transaction_id', $requestId)
                        ->where('trans_type', 'debit')
                        ->first();

                    $alreadyRefunded = WalletTransactions::where('transaction_id', $requestId)
                    ...
                    if ($walletTx) {
                        $user = User::where('id', $walletTx->user_id)->first();
                        ...
                        if ($user) {
                            ...
                            $walletTrans->amount = $refundAmount;
                            $walletTrans->service = $walletTx->service;
                            $walletTrans->status = 'refunded';
                            $walletTrans->transaction_id = $requestId;
                            $walletTrans->balance_before = $balanceBefore;
                            $walletTrans->balance_after = $balanceAfter;
                            $walletTrans->save();
                        } else {
                            Log::warning('VTpass Webhook: User not found for refund', ['user_id' => $walletTx->user_id]);
                        }
                    } else {
                        Log::warning('VTpass Webhook: Wallet transaction not found for refund', ['transaction_id' => $requestId]);
                    }
                }
                ...
                Log::warning('VTpass Webhook: Transaction not found', ['transaction_id' => $requestId]);
                // If transaction not found, still return success to prevent VTpass retry flood
                return response()->json(['response' => 'success']);
            }
        }

        Log::info('VTpass Webhook: Non-transaction-update type received', ['type' => $type]);
        return response()->json(['response' => 'success']);
    }
