...
class NinePsbWebhookController extends Controller
{
    ...
    public function handle(Request $request, PercentageService $percentageService)
    {
        ...
        if (
        !is_string($expectedUsername) || !is_string($expectedPassword) ||
        !hash_equals($expectedUsername, (string) $providedUsername) ||
        !hash_equals($expectedPassword, (string) $providedPassword)
        ) {
